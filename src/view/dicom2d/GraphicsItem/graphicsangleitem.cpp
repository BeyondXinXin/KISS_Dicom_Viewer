...
void GraphicsAngleItem::nextStage()
{
    ...
    case First:
        ...
        break;
    case Second:
        if (point2->pos() != pointAngle->pos() && point2->pos() != point1->pos()) {
            currentStage = Final;
        }
        break;
    }
...
void GraphicsAngleItem::updateTextItem()
{
    ...
    double angle = acos(QPointF::dotProduct(vector1, vector2) / sqrt((vector1.x() * vector1.x() + vector1.y() * vector1.y()) * (vector2.x() * vector2.x() + vector2.y() * vector2.y())));
...
QPointF GraphicsAngleItem::textItemPos()
{
    ...
    if ((vector1.x() < 0 || vector1.y() < 0) && (vector2.x() < 0 || vector2.y() < 0)) {
        ...
        return QPointF(pointAngle->pos().x() - (pointAngle->crossSize().width() + textItem->boundingRect().width()),
                       pointAngle->y());
    }
}
