    ...
    foreach (QString file, image_files_) {
        ...
        if (generate < frameCount) {
            for (; generate < generate_end; generate++) {
                ...
                QString filepath = dest_dir_ + QDir::separator() + QString("%1%2%3.%4").arg(fileName, frameCount > 1 ? QString::number(generate) : QString(), this->leaflet_ ? "(" + QString::number(generate) + ")" : "", suffix);
