...
void InitialDao()
{
    if (!DbManager::DbInitial() || !DbManager::CreateDbFile()) {
...
string QUIHelper::String2UTF8(const string & t_str)
{
    int nwLen = ::MultiByteToWideChar(CP_ACP, 0, t_str.c_str(), -1, NULL, 0);
    wchar_t * pwBuf = new wchar_t[nwLen + 1];
    ZeroMemory(pwBuf, nwLen * 2 + 2);
    ::MultiByteToWideChar(CP_ACP, 0, t_str.c_str(), t_str.length(), pwBuf, nwLen);
