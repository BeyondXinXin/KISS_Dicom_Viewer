    ...
    case mdAdd:
        if (ioApi == NULL) {
            ...
            p->zipFile_f = zipOpen3(ioDevice,
                                    mode == mdCreate ? APPEND_STATUS_CREATE : mode == mdAppend ? APPEND_STATUS_CREATEAFTER
...
void QuaZip::close()
{
    ...
    case mdAppend:
    case mdAdd:
        p->zipError = zipClose(p->zipFile_f, p->comment.isNull() ? NULL : isUtf8Enabled() ? p->comment.toUtf8().constData()
...
QString QuaZip::getComment() const
{
    ...
    return (unzGetFileFlags(p->unzFile_f, &flags) == UNZ_OK) && (flags & UNZ_ENCODING_UTF8)
      ...
      : p->commentCodec->toUnicode(comment);
}
...
QString QuaZip::getCurrentFileName() const
{
    ...
    QString result = (file_info.flag & UNZ_ENCODING_UTF8)
      ...
      : p->fileNameCodec->toUnicode(fileName);
    if (result.isEmpty())
        return result;
    // Add to directory map
